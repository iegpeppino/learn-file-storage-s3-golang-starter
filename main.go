...
// Not needed since we store thumbnails as strings in db
...
func main() {
	...
	if err != nil {
		respondWithError(nil, http.StatusInternalServerError, "Couldn't set AWS configuration file", err)
		return
	}
	...
	// Creating s3 client and setting to cfg
	// NewFromConfig returns a *s3.Config struct
